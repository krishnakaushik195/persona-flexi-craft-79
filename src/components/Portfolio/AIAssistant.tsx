...
User question: ${userMessage}`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!apiKey) {
      const errorMessage: Message = {
        role: 'assistant',
        content: 'Please configure your Gemini API key in edit mode first.',
        timestamp: new Date()
      };
  ...
  return (
    <div className="h-full flex flex-col">
        ...
        <div className="bg-secondary/30 rounded-xl p-4 mb-4">
          <div className="flex items-start space-x-3">
            ...
            <p className="text-sm text-portfolio-text-muted">
              Hi! I'm your AI assistant. Please configure your Gemini API key in edit mode to start chatting!
            </p>
          </div>
        </div>
      )}
      ...
      {messages.length > 0 && (
        <ScrollArea className="flex-1 bg-secondary/30 rounded-xl p-4 mb-4">
      ...
      <form onSubmit={handleSubmit} className="flex space-x-2">
        <Input
          ...
          placeholder={apiKey ? "Ask about this portfolio..." : "Configure API key in edit mode first"}
